...
class Serializer
{
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        if (null === $data) {
            ...
            $subClass = substr($class, 4, -1);
            $deserialized = [];

            if (strrpos($subClass, ",") !== false) {
                $subClass_array = explode(',', $subClass, 2);
            ...
            // this graceful.
            if (is_object($data)) {
                return !empty($data->date) ? new \DateTime($data->date) : null;
            }
            elseif (!empty($data)) {
            ...
            foreach ($instance::getObjectAttributes() as $property => $type) {

                if (!property_exists($data, $property)) {
                    continue;
                }

                $instance->$property = self::deserialize($data->{$property}, $type, null);
            }
